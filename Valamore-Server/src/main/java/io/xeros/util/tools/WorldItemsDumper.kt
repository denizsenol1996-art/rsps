    ...
    private fun getItem(doc : Document) {
        ...
        try {
            itemID = if(!itemString.contains(",")){
                ...
                StringUtils.substringBetween(doc.html(),"Item ID","</td>").replace("<td colspan=13>","").replace("dataattrparamid","").replace(Regex("[^\\|$£0-9,]"),"").split(",").get(0).toInt()
            }
        }catch (e : Exception) {
        ...
        doc.select("a[href][title]").forEach {
            ...
            if (title.contains("Edit section: spawn",true)) {
                ...
                if (link.contains("section",true)) {
                    ...
                    val doc = Jsoup.connect(editLink).get()

                    val body = doc.text().replace("{{FloorNumber|uk=1}}","")
