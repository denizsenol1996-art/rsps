...
public class Region {
    ...
    public void removeObject(int objectId, int x, int y, int height, int type, int direction) { //TODO
        var def = CacheManager.INSTANCE.objects(objectId);
        int xLength;
        int yLength;
        ...
        if (type == 22) {
            ...
            if (def.getSolid() != 0) {
                setClippingForVariableObject(x, y, height, type, direction, def.getImpenetrable(), true);
                if (def.getClipped()) {
    ...
    public void addObject(int objectId, int x, int y, int height, int type, int direction) {
        ObjectDefinition definition = CacheManager.INSTANCE.objects(objectId);
        int xLength;
        int yLength;
        if (direction == 1 || direction == 3) {
            xLength = definition.getSizeY();
            yLength = definition.getSizeX();
        } else {
            xLength = definition.getSizeX();
            yLength = definition.getSizeY();
        }
        if (type == 22) {
            if (definition.getSolid() == 1 || definition.getImpenetrable() || definition.getObstructive()) {
                addClipping(x, y, height, 262144);
            }
        } else if (type >= 9 && type <= 21) {
            if (definition.getSolid() != 0) {
                addClipping(x, y, height, xLength, yLength, definition.getImpenetrable(), false);
                if (definition.getImpenetrable()) {
                    addClipping(x, y, height, xLength, yLength, true, true);
                }
            }
        } else if (type >= 0 && type <= 3) {
            if (definition.getSolid() != 0) {
                addVariableClipping(x, y, height, type, direction, definition.getImpenetrable(), false);
                if (definition.getImpenetrable()) {
                    addVariableClipping(x, y, height, type, direction, true, true);
                }
            }
        }
    }
    ...
    public void addClipping(int x, int y, int z, int xLength, int yLength, boolean solid, boolean projectile) {
        int mask = 256;
