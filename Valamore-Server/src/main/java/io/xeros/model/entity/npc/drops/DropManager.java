...
public class DropManager {
    ...
    public void handle(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        ...
        if (Misc.random(80) == 10 || Server.isDebug()) {
            if (player.getItems().getItemCount(5509, false) == 1 && player.getItems().getItemCount(5510, false) == 0) {
                player.sendMessage("You receive a {}.", CacheManager.INSTANCE.item(5510).getName());
                ...
                player.sendMessage("You receive a {}.", CacheManager.INSTANCE.item(5512).getName());
    ...
    private boolean writeItem(Player player, int itemId, int minAmount, int maxAmount, String rarityString, int dropChance) {
        ...
        var def = CacheManager.INSTANCE.item(itemId);
        var itemName = CacheManager.INSTANCE.item(itemId).getName();
        var id = itemId;
        if (def.getNoted()) {
            id = itemId - 1;
            itemName = CacheManager.INSTANCE.item(id).getName();
        }
