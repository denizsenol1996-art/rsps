...
buildscript {
    dependencies {
        classpath(fileTree(mapOf("dir" to "deps", "include" to listOf("*.jar"))))
    }
}
...
dependencies {
    ...
    implementation(fileTree(mapOf("dir" to "deps", "include" to listOf("*.jar"))))
    implementation(lib.logback.classic)
    annotationProcessor("org.projectlombok:lombok:1.18.32")
    // https://mvnrepository.com/artifact/com.github.petitparser.java-petitparser/petitparser-core
...
tasks.withType<JavaCompile>().all {
    options.apply {
        options.compilerArgs.addAll(listOf("--enable-preview", "-nowarn", "-Xlint:none"))
    }
}
